...
import { Bot, Send, Sparkles, User, CheckCircle2, ShieldCheck, Zap, ArrowRight } from 'lucide-react';
...
export const CopilotPanel: React.FC<CopilotPanelProps> = () => {
  ...
  const handleSend = async (customPrompt?: string) => {
    ...
    if (apiResult && apiResult.data) {
      ...
      // Local AI Copilot intelligent fallback logic with zero hallucination
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          {
    ...
    <Card className="mb-8">
      <CardHeader>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <span className="text-xs px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-semibold border border-emerald-500/20 flex items-center space-x-1 self-start sm:self-auto">
      ...
      <CardContent>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {samplePrompts.map((p, i) => (
            <button
              ...
              className="text-xs px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800/80 hover:bg-emerald-500/10 hover:text-emerald-600 dark:hover:text-emerald-400 border border-slate-200 dark:border-slate-700/60 transition-colors flex items-center space-x-1.5"
            >
        ...
        <div className="space-y-4 mb-4 max-h-[480px] overflow-y-auto p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800">
          {messages.map((msg) => (
            ...
            >
              {msg.sender === 'assistant' && (
                <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-emerald-500 via-teal-500 to-cyan-500 flex items-center justify-center text-white shrink-0 shadow-md">
                  ...
                  <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-800 space-y-3">
                    {msg.structuredData.aiTitle && (
                      <div className="flex items-center justify-between">
                        <h5 className="text-xs font-extrabold font-heading text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">
                          {msg.structuredData.aiTitle}
                        </h5>
                    ...
                    {msg.structuredData.actionItems && (
                      <div className="space-y-2">
                        {msg.structuredData.actionItems.map((item: any, idx: number) => (
                          ...
                          >
                            <span className="w-5 h-5 rounded-full bg-emerald-500 text-white font-extrabold flex items-center justify-center text-[10px] shrink-0 mt-0.5">
                              {item.step || idx + 1}
                            </span>
                    ...
                    {msg.structuredData.totalImpact && (
                      <div className="p-3 rounded-xl bg-gradient-to-r from-emerald-950/40 via-teal-950/40 to-slate-900 border border-emerald-500/30 text-xs flex flex-wrap items-center justify-between gap-2 font-mono font-bold">
          ...
          {isAsking && (
            <div className="flex space-x-3 items-center text-slate-400 text-xs font-mono">
        ...
        <div className="flex items-center space-x-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500"
          />
          <Button onClick={() => handleSend()} className="flex items-center space-x-1 shrink-0">
            <span>Ask AI</span>
