    ...
    <Card className="mb-8 theme-transition">
      ...
      <CardContent>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="p-6 rounded-2xl bg-slate-900 text-white border border-slate-800 shadow-2xl relative overflow-hidden">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-mono text-emerald-400 uppercase font-bold tracking-widest flex items-center space-x-1.5">
            ...
            <div className="relative h-64 rounded-xl bg-slate-950 border border-slate-800 p-4 overflow-hidden">
              {/* Grid Lines */}
              ...
              <div
                ...
                className={`absolute top-6 left-8 p-3 rounded-xl border cursor-pointer transition-all ${
                  selectedFacilityNode === 'polymer'
                    ? 'bg-blue-950/80 border-blue-500 shadow-lg shadow-blue-500/30 ring-2 ring-blue-500/50'
                    : 'bg-slate-900/80 border-slate-800 hover:border-slate-600'
                }`}
              >
                <div className="w-3 h-3 rounded-full bg-blue-500 mb-1" />
                <span className="text-[10px] font-bold text-white block">Resin Storage</span>
                <span className="text-[9px] text-slate-400 font-mono">2.80 tCO₂/T</span>
              </div>
              ...
              <div
                ...
                className={`absolute top-12 left-44 p-4 rounded-xl border cursor-pointer transition-all ${
                  selectedFacilityNode === 'furnace'
                    ? 'ambient-halo-danger bg-rose-950/90 border-rose-500 shadow-2xl'
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-center space-x-1.5 mb-1">
                    ...
                    <span className="text-[10px] font-extrabold font-mono text-rose-400 uppercase">
                      RED ALERT (1418°C)
                    </span>
                  </div>
              ...
              <div
                ...
                className={`absolute bottom-8 left-36 p-3 rounded-xl border cursor-pointer transition-all ${
                  selectedFacilityNode === 'extrusion'
                    ? 'bg-amber-950/80 border-amber-500 shadow-lg shadow-amber-500/30'
                    : 'bg-slate-900/80 border-slate-800'
                }`}
              >
                <div className="w-3 h-3 rounded-full bg-amber-500 mb-1" />
                <span className="text-[10px] font-bold text-white block">Extrusion Motors</span>
                <span className="text-[9px] text-amber-400 font-mono">25.0 tCO₂/mo</span>
              </div>
              ...
              <div
                ...
                className={`absolute bottom-6 right-8 p-3 rounded-xl border cursor-pointer transition-all ${
                  selectedFacilityNode === 'scrap'
                    ? 'ambient-halo-danger bg-rose-950/80 border-rose-500'
                    : 'bg-slate-900/80 border-slate-800'
                }`}
              >
                <div className="w-3 h-3 rounded-full bg-purple-500 mb-1" />
                <span className="text-[10px] font-bold text-white block">Trim Scrap Yard</span>
                <span className="text-[9px] text-purple-400 font-mono">17.0 tCO₂/mo</span>
              </div>
            </div>

            <div className="mt-3 text-[11px] text-slate-400 font-mono flex justify-between">
              <span>Selected Node: <strong className="text-white uppercase">{selectedFacilityNode}</strong></span>
              <span className="text-rose-400 font-bold">Max Hotspot: Furnace Burner Unit</span>
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
            ...
            {selectedPoint !== null && (
              <div className="mt-4 p-3.5 rounded-xl bg-slate-900 text-slate-100 border border-slate-800 text-xs font-mono flex justify-between items-center">
                <div>
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-r from-rose-950/20 via-slate-900/60 to-purple-950/20 border border-rose-500/30">
                ...
                {activeAnomaly.featureImportance.map((f, idx) => (
                  <div key={idx} className="space-y-1 text-xs">
                    <div className="flex justify-between font-mono text-[11px]">
                      ...
                      <span className="text-emerald-400 font-bold">{(f.weight * 100).toFixed(0)}%</span>
                    </div>
